    ...
    <div class="pay-container">
      ...
      <div class="checkout-steps">
        ...
        <div class="submit">
          ...
          <a href="javasrcipt:;" class="btn" @click="pay">立即支付</a>
        </div>
        <div class="otherpay">
          <div class="step-tit">
            <h5>其他支付方式</h5>
  ...
  methods: {
    ...
    async pay(){
      ...
      try {
        ...
        //2.弹出一个消息框出去展示二维码图片
        this.$alert(`<img src="${imgUrl}" />`, "请使用微信使用",{
          ...
          //3. 点击按钮之后的处理及和第三步产生联系
          beforeClose:(action, instance, done) => {
            //关闭之前回调
            //如果不写这个回调, 那么无论点击什么按钮,消息盒子都会强制关闭
            //如果写了这个回调,那么消息盒子由我们自己控制
            if(action === "confirm"){
              //真实环境, 需要用钱的
              // if(this.status !== 200){
              //   this.$message.warning('没有支付哦!!!')
              // }
              ...
              this.$message.warning("需要联系客服处理哦!!!")
        ...
        // 3. 弹出消息框同时循环的给后台发请求, 获取该订单的支付状态数据
        //根据返回来的支付转台数据区决定要不要跳转到支付成功页面
        if(!this.timer){
